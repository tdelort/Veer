...
#define ELEM_ACCESS_DECLARE(_elem, _index)											\
	[[nodiscard]] constexpr TYPE _elem () const requires veer::greater<ELEM_COUNT, _index>; 	\
	constexpr TYPE& _elem () requires veer::greater<ELEM_COUNT, _index>;								

#define ELEM_ACCESS_DEFINE(_elem, _index)																	\
	template<arithmetic TYPE, size_t ELEM_COUNT>															\
	constexpr TYPE vec<TYPE, ELEM_COUNT>:: _elem () const requires veer::greater<ELEM_COUNT, _index> 	\
	{ 																										\
		return m_data[_index]; 																				\
	}																										\
	template<arithmetic TYPE, size_t ELEM_COUNT>															\
	constexpr TYPE& vec<TYPE, ELEM_COUNT>:: _elem () requires veer::greater<ELEM_COUNT, _index> 						\
	{ 																										\
		return m_data[_index]; 																				\
	}																										
...
namespace veer::math
{
    ...
    template<arithmetic TYPE, size_t ELEM_COUNT>
	[[nodiscard]] constexpr vec<TYPE, ELEM_COUNT> operator+(const vec<TYPE, ELEM_COUNT>& _lhs, const vec<TYPE, ELEM_COUNT>& _rhs);
    ...
    template<arithmetic TYPE, size_t ELEM_COUNT>
	[[nodiscard]] constexpr vec<bool, ELEM_COUNT> operator==(const vec<TYPE, ELEM_COUNT>& _lhs, const vec<TYPE, ELEM_COUNT>& _rhs);
	...
	template<arithmetic TYPE, size_t ELEM_COUNT>
	[[nodiscard]] constexpr vec<TYPE, ELEM_COUNT> normalize_safe(const vec<TYPE, ELEM_COUNT>& _v, float _epsilon = FLT_EPSILON);
